...
<body>
			...
			<tbody id="reviewTbody">
				<c:forEach var="r" items="${list }">
					<tr>
						...
						<td style="color: #404040" class="col-6">[문의]
							${r.prReviewTitle }</td>
					...
					<td colspan="5">
						<ul class="pagination">
							...
							<c:if test="${pageVO.prev }">
								<li><a
									href="reviewSearchPageList.do?reSelectOpt=${key}&reviewInfo=${val}&pageNum=${pageVO.startPage - 1 }&amount=${pageVO.amount}">이전</a></li>
							</c:if>
							...
							<c:forEach var="num" begin="${pageVO.startPage }"
								...
								<li class="${pageVO.pageNum eq num ? 'active' : '' }"><a
									href="reviewSearchPageList.do?reSelectOpt=${key}&reviewInfo=${val}&pageNum=${num }&amount=${pageVO.amount}">${num }</a></li>
							</c:forEach>

							<!-- 3. 다음버튼 활성화 여부 -->
							<c:if test="${pageVO.next }">
								<li><a
									href="reviewSearchPageList.do?reSelectOpt=${key}&reviewInfo=${val}&pageNum=${pageVO.endPage + 1 }&amount=${pageVO.amount}">다음</a></li>
							</c:if>
						</ul>
					</td>
				</tr>
	...
	document.addEventListener('DOMContentLoaded', function() {
		...
		btn.addEventListener('click', lookInfo)
	 ...
	 function lookInfo() {
	    ...
	    selectFrm.action='reviewSearchPageList.do?reSelectOpt=' + select + '&reviewInfo=' + val
	    selectFrm.submit();
	 }
