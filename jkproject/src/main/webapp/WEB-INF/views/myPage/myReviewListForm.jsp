...
<body>
		...
		<table class="table" id="reviewTable" style="text-align: center">
			...
			<tbody id="reviewTbody">
				<c:forEach var="r" items="${list }">
				  <c:if test="${memberId eq r.memberId }">
					<tr>
					...
					<td colspan="5">
						<ul class="pagination">
							...
							<c:if test="${pageVO.prev }">
								<li><a
									href="myReviewPageList.do?memberId=${memberId }&pageNum=${pageVO.startPage - 1 }&amount=${pageVO.amount}">이전</a></li>
							</c:if>
							...
							<c:forEach var="num" begin="${pageVO.startPage }"
								...
								<li class="${pageVO.pageNum eq num ? 'active' : '' }"><a
									href="myReviewPageList.do?memberId=${memberId }&pageNum=${num }&amount=${pageVO.amount}">${num }</a></li>
							</c:forEach>

							<!-- 3. 다음버튼 활성화 여부 -->
							<c:if test="${pageVO.next }">
								<li><a
									href="myReviewPageList.do?memberId=${memberId }&pageNum=${pageVO.endPage + 1 }&amount=${pageVO.amount}">다음</a></li>
							</c:if>
						</ul>
					</td>
				</tr>
	...
	<script src="js/myReviewListForm.js"></script>
